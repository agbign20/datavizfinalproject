...
ui <- fluidPage(
  tabsetPanel(
    tabPanel("1988", 
             sidebarLayout(
               sidebarPanel(h1(strong("MLB Dataset")),
                            hr(),
                            br(),
...
server <- function(input, output, session) {
  ...
  mlbpositions_88_sub <- reactive({
    mlbpositions_88 %>% filter(Team == input$TeamChoice88)
  })
  
  mlbpositions_89_sub <- reactive({
    mlbpositions_89 %>% filter(Team == input$TeamChoice89) %>%
      mutate(Position = fct_reorder(Position, nPlayers))
  })
  
  mlbpositions_90_sub <- reactive({
    mlbpositions_90 %>% filter(Team == input$TeamChoice90)
  })
  
  mlbpositions_91_sub <- reactive({
    mlbpositions_91 %>% filter(Team == input$TeamChoice91)
  })
  
  mlbpositions_92_sub <- reactive({
    mlbpositions_92 %>% filter(Team == input$TeamChoice92)
  })
  
  mlbpositions_93_sub <- reactive({
    mlbpositions_93 %>% filter(Team == input$TeamChoice93)
  })
  
  mlbpositions_94_sub <- reactive({
    mlbpositions_94 %>% filter(Team == input$TeamChoice94)
  })
  
  mlbpositions_95_sub <- reactive({
    mlbpositions_95 %>% filter(Team == input$TeamChoice95)
  })
  
  mlbpositions_96_sub <- reactive({
    mlbpositions_96 %>% filter(Team == input$TeamChoice96)
  })
  
  mlbpositions_97_sub <- reactive({
    mlbpositions_97 %>% filter(Team == input$TeamChoice97)
  })
  
  mlbpositions_98_sub <- reactive({
    mlbpositions_98 %>% filter(Team == input$TeamChoice98)
  })
  
  mlbpositions_99_sub <- reactive({
    mlbpositions_99 %>% filter(Team == input$TeamChoice99)
  })
  
  mlbpositions_00_sub <- reactive({
    mlbpositions_00 %>% filter(Team == input$TeamChoice00)
  })
  
  mlbpositions_01_sub <- reactive({
    mlbpositions_01 %>% filter(Team == input$TeamChoice01)
  })
  
  mlbpositions_02_sub <- reactive({
    mlbpositions_02 %>% filter(Team == input$TeamChoice02)
  })
  
  mlbpositions_03_sub <- reactive({
    mlbpositions_03 %>% filter(Team == input$TeamChoice03)
  })
  
  mlbpositions_04_sub <- reactive({
    mlbpositions_04 %>% filter(Team == input$TeamChoice04)
  })
  
  mlbpositions_05_sub <- reactive({
    mlbpositions_05 %>% filter(Team == input$TeamChoice05)
  })
  
  mlbpositions_06_sub <- reactive({
    mlbpositions_06 %>% filter(Team == input$TeamChoice06)
  })
  
  mlbpositions_07_sub <- reactive({
    mlbpositions_07 %>% filter(Team == input$TeamChoice07)
  })
  
  mlbpositions_08_sub <- reactive({
    mlbpositions_08 %>% filter(Team == input$TeamChoice08)
  })
  
  mlbpositions_09_sub <- reactive({
    mlbpositions_09 %>% filter(Team == input$TeamChoice09)
  })
  
  mlbpositions_10_sub <- reactive({
    mlbpositions_10 %>% filter(Team == input$TeamChoice10)
  })
  
  mlbpositions_11_sub <- reactive({
    mlbpositions_11 %>% filter(Team == input$TeamChoice11)
  })
  
  mlbpositions_cust_sub <- reactive({
    mlbpositions_cust %>% filter(Team == input$TeamChoiceCust,
                                 Year == input$ChooseYear) %>%
      slice(1:25)
  })
  ...
  col_plot_88 <- reactive({
    ggplot(data = mlbsalary_88_sub(),
           aes(x = Player,
               y = Salary)) + geom_col(color = "black", fill = "white") + 
  ...
  graph_plot_88 <- reactive({
    ggplot(data = mlbpositions_88_sub(),
  ...
  graph_plot_89 <- reactive({
    ggplot(data = mlbpositions_89_sub(),
           aes(x = Position, y = nPlayers)) + geom_bar(color = "black", fill = "white", stat = "identity") + 
      coord_flip() + ylab("Number of Players")
  })
  
  graph_plot_90 <- reactive({
    ggplot(data = mlbpositions_90_sub(),
           aes(x = Position, y = nPlayers)) + geom_bar(color = "black", fill = "white", stat = "identity") + 
      coord_flip() + ylab("Number of Players")
  })
  
  graph_plot_91 <- reactive({
    ggplot(data = mlbpositions_91_sub(),
           aes(x = Position, y = nPlayers)) + geom_bar(color = "black", fill = "white", stat = "identity") + 
      coord_flip() + ylab("Number of Players")
  })
  
  graph_plot_92 <- reactive({
    ggplot(data = mlbpositions_92_sub(),
           aes(x = Position, y = nPlayers)) + geom_bar(color = "black", fill = "white", stat = "identity") + 
      coord_flip() + ylab("Number of Players")
  })
  
  graph_plot_93 <- reactive({
    ggplot(data = mlbpositions_93_sub(),
           aes(x = Position, y = nPlayers)) + geom_bar(color = "black", fill = "white", stat = "identity") + 
      coord_flip() + ylab("Number of Players")
  })
  
  graph_plot_94 <- reactive({
    ggplot(data = mlbpositions_94_sub(),
           aes(x = Position, y = nPlayers)) + geom_bar(color = "black", fill = "white", stat = "identity") + 
      coord_flip() + ylab("Number of Players")
  })
  
  graph_plot_95 <- reactive({
    ggplot(data = mlbpositions_95_sub(),
           aes(x = Position, y = nPlayers)) + geom_bar(color = "black", fill = "white", stat = "identity") + 
      coord_flip() + ylab("Number of Players")
  })
  
  graph_plot_96 <- reactive({
    ggplot(data = mlbpositions_96_sub(),
           aes(x = Position, y = nPlayers)) + geom_bar(color = "black", fill = "white", stat = "identity") + 
      coord_flip() + ylab("Number of Players")
  })
  
  graph_plot_97 <- reactive({
    ggplot(data = mlbpositions_97_sub(),
           aes(x = Position, y = nPlayers)) + geom_bar(color = "black", fill = "white", stat = "identity") + 
      coord_flip() + ylab("Number of Players")
  })
  
  graph_plot_98 <- reactive({
    ggplot(data = mlbpositions_98_sub(),
           aes(x = Position, y = nPlayers)) + geom_bar(color = "black", fill = "white", stat = "identity") + 
      coord_flip() + ylab("Number of Players")
  })
  
  graph_plot_99 <- reactive({
    ggplot(data = mlbpositions_99_sub(),
           aes(x = Position, y = nPlayers)) + geom_bar(color = "black", fill = "white", stat = "identity") + 
      coord_flip() + ylab("Number of Players")
  })
  
  graph_plot_00 <- reactive({
    ggplot(data = mlbpositions_00_sub(),
           aes(x = Position, y = nPlayers)) + geom_bar(color = "black", fill = "white", stat = "identity") + 
      coord_flip() + ylab("Number of Players")
  })
  
  graph_plot_01 <- reactive({
    ggplot(data = mlbpositions_01_sub(),
           aes(x = Position, y = nPlayers)) + geom_bar(color = "black", fill = "white", stat = "identity") + 
      coord_flip() + ylab("Number of Players")
  })
  
  graph_plot_02 <- reactive({
    ggplot(data = mlbpositions_02_sub(),
           aes(x = Position, y = nPlayers)) + geom_bar(color = "black", fill = "white", stat = "identity") + 
      coord_flip() + ylab("Number of Players")
  })
  
  graph_plot_03 <- reactive({
    ggplot(data = mlbpositions_03_sub(),
           aes(x = Position, y = nPlayers)) + geom_bar(color = "black", fill = "white", stat = "identity") + 
      coord_flip() + ylab("Number of Players")
  })
  
  graph_plot_04 <- reactive({
    ggplot(data = mlbpositions_04_sub(),
           aes(x = Position, y = nPlayers)) + geom_bar(color = "black", fill = "white", stat = "identity") + 
      coord_flip() + ylab("Number of Players")
  })
  
  graph_plot_05 <- reactive({
    ggplot(data = mlbpositions_05_sub(),
           aes(x = Position, y = nPlayers)) + geom_bar(color = "black", fill = "white", stat = "identity") + 
      coord_flip() + ylab("Number of Players")
  })
  
  graph_plot_06 <- reactive({
    ggplot(data = mlbpositions_06_sub(),
           aes(x = Position, y = nPlayers)) + geom_bar(color = "black", fill = "white", stat = "identity") + 
      coord_flip() + ylab("Number of Players")
  })
  
  graph_plot_07 <- reactive({
    ggplot(data = mlbpositions_07_sub(),
           aes(x = Position, y = nPlayers)) + geom_bar(color = "black", fill = "white", stat = "identity") + 
      coord_flip() + ylab("Number of Players")
  })
  
  graph_plot_08 <- reactive({
    ggplot(data = mlbpositions_08_sub(),
           aes(x = Position, y = nPlayers)) + geom_bar(color = "black", fill = "white", stat = "identity") + 
      coord_flip() + ylab("Number of Players")
  })
  
  graph_plot_09 <- reactive({
    ggplot(data = mlbpositions_09_sub(),
           aes(x = Position, y = nPlayers)) + geom_bar(color = "black", fill = "white", stat = "identity") + 
      coord_flip() + ylab("Number of Players")
  })
  
  graph_plot_10 <- reactive({
    ggplot(data = mlbpositions_10_sub(),
           aes(x = Position, y = nPlayers)) + geom_bar(color = "black", fill = "white", stat = "identity") + 
      coord_flip() + ylab("Number of Players")
  })
  
  graph_plot_11 <- reactive({
    ggplot(data = mlbpositions_11_sub(),
           aes(x = Position, y = nPlayers)) + geom_bar(color = "black", fill = "white", stat = "identity") + 
      coord_flip() + ylab("Number of Players")
  })
  
  graph_plot_cust <- reactive({
    ggplot(data = mlbpositions_cust_sub(),
           aes(x = Position, y = nPlayers)) + geom_bar(color = "black", fill = "white", stat = "identity") +
      coord_flip() + ylab("Number of Players")
  })
  ...
  output$graphposition_11 <- renderPlot({
    graph_plot_11()
  })
  
  output$graphposition_cust <- renderPlot({
    graph_plot_cust()
  })
